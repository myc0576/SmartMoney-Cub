...
def run_jev_doctor(*, root: str | Path | None = None) -> dict[str, Any]:
    ...
    from smartmoney_cub_harness.jev.connection import configured_backend
    direct_backend = configured_backend(root) if root is not None else TypeSafeDirectJevBackend()
